  ...
  async handleGrabClipButtonClick(button, platform) {
    try {
      ...
      if (videoPageInfo && videoPageInfo.url) {
        ...
        console.debug(
          `[base] Opening GrabClip in new tab: ${targetUrl}`,
        );

        // 打开新窗口
        window.open(targetUrl, "_blank");
      } else {
        console.debug(`[base] Failed to get video page info`);
      }
    } catch (error) {
      console.error(
        `[base] GrabClip button click error for ${platform.name}:`,
        error,
      );
    }
  }
        ...
        if (mutation.type === "childList" && mutation.addedNodes.length > 0) {
          ...
          for (let node of mutation.addedNodes) {
            if (node.nodeType === Node.ELEMENT_NODE) {
              // 1. 使用子类配置的选择器检查
              const selectorMatch = this.observerConfig.selectors.some(
                (selector) => {
